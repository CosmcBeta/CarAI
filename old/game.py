...
class Game:
    def __init__(self) -> None:
        ...
        _ = pygame.init()
        self.total_display: pygame.Surface = pygame.display.set_mode(
            constants.TOTAL_SIZE
        )
        self.screen: pygame.Surface = pygame.surface.Surface(constants.SCREEN_SIZE)
        self.bottom_bar: pygame.Surface = pygame.surface.Surface(
            constants.BOTTOM_BAR_SIZE
        )
        ...
        self.starting_position: tuple[int, int] = (
            self.track.final_points[0][0],
            self.track.final_points[0][1],
        )
        self.player: Car = Car(self.starting_position[0], self.starting_position[1])
        self.player_sprite: CameraGroup = CameraGroup()
        self.player_sprite.add(self.player)

        self.player.mask = pygame.mask.from_threshold(
            self.player.image, pygame.Color("green")
        )
        ...
        self.seed_textbox: TextBox = TextBox(
            (0, 0, 200, 50),
            command=self.set_seed,
            track_arg=self.track,
            clear_on_enter=True,
            inactive_on_enter=True,
        )
        self.seed_textbox.rect.center = (
            (constants.BB_WIDTH // 8) * 5,
            constants.S_HEIGHT + (constants.BB_HEIGHT // 2),
        )

        self.randomize_seed_button: Button = Button(
            (0, 0, 200, 50),
            pygame.Color(0, 0, 0),
            self.randomize_seed,
            track=self.track,
        ...
        self.randomize_seed_button.rect.center = (
            (constants.BB_WIDTH // 8) * 3,
            constants.S_HEIGHT + (constants.BB_HEIGHT // 2),
        )
        ...
        self.restart_button: Button = Button(
            ...
            x_position=self.starting_position[0],
            y_position=self.starting_position[1],
            text="Restart",
            **button_style,
        )
        self.restart_button.rect.center = (
            (constants.BB_WIDTH // 8),
            constants.S_HEIGHT + (constants.BB_HEIGHT // 2),
        )

        self.seed_button: Button = Button(
            (0, 0, 200, 50),
            pygame.Color(0, 0, 0),
            self.seed_textbox.execute,
            text="Set Seed",
            **button_style,
        )
        self.seed_button.rect.center = (
            (constants.BB_WIDTH // 8) * 7,
            constants.S_HEIGHT + (constants.BB_HEIGHT // 2),
        )
    ...
    def run(self):
        while self.running:
            ...
            self.restart_button.update_starting_pos(
                self.starting_position[0], self.starting_position[1]
            )

            velocity_surface = self.font.render(
                "{:.2f}".format(self.player.speed), False, (0, 0, 0)
            )
            position_text = (
                "{:.2f}".format(self.player.position.x)
                + ", "
                + "{:.2f}".format(self.player.position.y)
            )
            ...
            _ = self.screen.blit(
                seed_surface,
                (3, velocity_surface.get_height() + position_surface.get_height()),
            )
